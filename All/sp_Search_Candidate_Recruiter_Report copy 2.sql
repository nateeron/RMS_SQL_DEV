...
/****** Object:  StoredProcedure [dbo].[sp_Search_Candidate_Recruiter_Report]    Script Date: 12/23/2025 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
-- ProcedureName: [dbo].[sp_Search_Candidate_Recruiter_Report]
...
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[sp_Search_Candidate_Recruiter_Report]
...
BEGIN
	...
	BEGIN TRY
		-- Parse comma-separated values into table variables for optimized filtering
		...
		-- Parse comma-separated input parameters into table variables
		IF @Current_Position IS NOT NULL AND LEN(LTRIM(RTRIM(@Current_Position))) > 0
		BEGIN
		...
		IF @Current_Position_By_Com_ID IS NOT NULL AND LEN(LTRIM(RTRIM(@Current_Position_By_Com_ID))) > 0
		BEGIN
		...
		IF @Looking_Position IS NOT NULL AND LEN(LTRIM(RTRIM(@Looking_Position))) > 0
		BEGIN
		...
		IF @Looking_Position_By_Com_ID IS NOT NULL AND LEN(LTRIM(RTRIM(@Looking_Position_By_Com_ID))) > 0
		BEGIN
		...
		IF @Skill_By_Company_ID IS NOT NULL AND LEN(LTRIM(RTRIM(@Skill_By_Company_ID))) > 0
		BEGIN
		...
		IF @Map_Skill_ID IS NOT NULL AND LEN(LTRIM(RTRIM(@Map_Skill_ID))) > 0
		BEGIN
		...
		CTE_CandidateBase AS (
			SELECT  [CAN].[Candidate_ID]
					...
					,[Current_Position_By_Com] = ISNULL([CAN].[Current_Position_By_Com], 0)
					,[CAN].[Min_Exp_Salary]
					,[CAN].[Max_Exp_Salary] 
					,[CAN].[Is_Employee]
					,[CAN].[Is_Deleted]
			...
			INNER JOIN [Candidate].[dbo].[Candidate] CAN ON [CAN].[Candidate_ID] = [C].[Candidate_ID]
			INNER JOIN [Candidate].[dbo].[Show_Data] SD ON [SD].[Show_Data_ID] = [C].[Show_Data_ID]
		...
		CTE_Phone AS (
			...
			INNER JOIN [Mobile].[dbo].[Mobile_Category_Type] MCT ON [MCT].[Category_Type_ID] = [T].[Category_Type_ID]
			INNER JOIN [Mobile].[dbo].[Tel_Type] TT ON [TT].[Tel_Type_ID] = [T].[Tel_Type_ID]
		...
		Person_CTE AS (
			...
			LEFT JOIN [Address].[DBO].[Address] AD ON [AD].[Reference_ID] = [PER].[Person_ID] 
				AND [AD].[Address_Type_ID] = (SELECT TOP 1 [Address_Type_ID] FROM Config_CTE)
				AND [AD].[Category_Type_ID] = (SELECT TOP 1 [Address_Category_Type_ID] FROM Config_CTE)
		),
		...
		CurrentPosition_Resolved_CTE AS (
			SELECT [CAN].[Candidate_ID]
					...
					,[Resolved_Position_ID] = CASE WHEN [CAN].[Current_Position_By_Com] = 0 OR [CAN].[Current_Position_By_Com] IS NULL  
						...
						ELSE (
							SELECT TOP 1 [Position_ID_OF_Com] = (
		...
		MapSkill_System_CTE AS (
			SELECT [MSKILL].[Map_Skill_ID]
					...
					,(SELECT TOP 1 [Map_Skill_Type_System] FROM Config_CTE) AS [SKILL_TYPE]
		...
		MapSkill_Temp_CTE AS (
			SELECT [MSKILLT].[Map_Skill_Temp_ID] AS [Map_Skill_ID]
					...
					,(SELECT TOP 1 [Map_Skill_Type_Temp] FROM Config_CTE) AS [SKILL_TYPE] 
		...
		Pipeline_LastCreate_CTE AS (
			...
			INNER JOIN [Candidate].[dbo].[Candidate] CAN2 ON [CAN2].[Candidate_ID] = [LUC].[Candidate_ID]
		),
		...
		SELECT 
			...
			[SEARCH_Person_ID] = [CAN].[Person_ID],
			[SEARCH_Owner_ID] = [LUC].[Recruiter_ID],
			[SEARCH_Updated_By] = [PU].[Full_Name],
			...
			[SEARCH_Exprie_Date_STR] = CASE WHEN [MCP].[Exprie_Date] IS NULL 
									...
									END,
			[SEARCH_Show_Data_ID] = [CAN].[Show_Data_ID],
			[SEARCH_Show_Data_Name] = [CAN].[Show_Data_Name]
		FROM CTE_CandidateBase CAN
		...
		LEFT JOIN Log_Update_Candidate_CTE LUC ON [LUC].[Candidate_ID] = [CAN].[Candidate_ID]
		...
		AND (
			@Company_ID = 0
			OR (
				NOT EXISTS (SELECT 1 FROM @SkillByCompanyTable)
		...
		AND (
			@Company_ID <> 0
			OR (
				NOT EXISTS (SELECT 1 FROM @MapSkillTable)
