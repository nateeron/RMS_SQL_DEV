...
-- Closed Position = Is_Active = 0 (from Project_Position table)
 -- [dbo].[sp_Get_Report_PositionStatus_Summary] 3357,'3,42,1063','2024-12-18','2025-01-08'
-- =============================================
...
BEGIN
	...
	WHERE 
		...
		(
			...
			OR (@DateFrom IS NULL AND @DateTo IS NOT NULL
				AND Created_Date < DATEADD(DAY, 1, @DateTo))
			OR (@DateFrom IS NOT NULL AND @DateTo IS NOT NULL
				AND Created_Date >= @DateFrom
				AND Created_Date < DATEADD(DAY, 1, @DateTo))
		)
		...
		AND (@Owner_ID = '' OR @Owner_ID IS NULL OR 
