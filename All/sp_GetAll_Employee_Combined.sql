USE [Employee]
GO
/****** Object:  StoredProcedure [dbo].[sp_GetAll_Employee_Combined]    Script Date: 12/19/2025 2:39:02 PM ******/
SET ANSI_NULLS ON
GO
...
--	@Owner_ID_str = '3869,3864',
--	@Client_ID_str = '3396,3399',
--	@Emp_Person_ID = '3867,5000',
...
--	@Owner_ID_str = '',
--	@Client_ID_str = '',
--	@Emp_Person_ID = '3867,5000' , --'M09 (K.) M09,htrfuh qqqqq',
--	@Employee_Status_id = 'Active,Released',
--	@Contract_Type_id = ''
----------------------------------------
-- From
...
BEGIN TRY
	...
	-- CTE: Split Person ID values
	EmployeeNameFilter AS (
		SELECT LTRIM(RTRIM([value])) AS [Employee_Name_Value]
		FROM STRING_SPLIT(@Emp_Person_ID, ',')
		WHERE @Emp_Person_ID <> '' AND @Emp_Person_ID IS NOT NULL
	),
	...
	ContractsWithPosition AS (
		SELECT 
			[CR].*,
			[CP].[Position_ID_Resolved] AS [Position_ID],
			[AP].[Position_Name]
		FROM ContractsWithRefer CR
	...
	WHERE [EMP].[Company_ID] = @Company_ID
		...
		-- Filter by Person ID (if provided) - supports multiple values
		AND (@Emp_Person_ID = '' OR @Emp_Person_ID IS NULL OR 
			EXISTS (
				SELECT 1 FROM EmployeeNameFilter ENF
				WHERE [CN].[Person_ID] = TRY_CAST(ENF.[Employee_Name_Value] AS INT)
			)
		)
		...
		AND (@Position_ID_str = '' OR @Position_ID_str IS NULL OR 
			(
				([ConEMP].[Position_ID_OF_Com] != 0 AND [ConEMP].[Position_ID_OF_Com] IS NOT NULL 
				 AND [ConEMP].[Position_ID_OF_Com] IN (SELECT [Position_ID_Value] FROM PositionIDFilter))
				OR
				(([ConEMP].[Position_ID_OF_Com] = 0 OR [ConEMP].[Position_ID_OF_Com] IS NULL) 
				 AND [CONTR].[Position_ID] IN (SELECT [Position_ID_Value] FROM PositionIDFilter))
			)
		);
--	client_CompanyID
--3396
