USE [Pipeline]
GO
/****** Object:  StoredProcedure [dbo].[sp_Get_Report_ClientProjectPosition_Combined]    Script Date: 12/19/2025 10:22:36 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	WHERE 
		...
		-- Filter by Position ID (if provided) - supports multiple values
		AND (@Position_Name = '' OR @Position_Name IS NULL OR 
			EXISTS (
				SELECT 1 FROM PositionFilter PF
				WHERE [PP].[Position_ID] = TRY_CAST(PF.[Position_Name_Value] AS INT)
			)
		)
