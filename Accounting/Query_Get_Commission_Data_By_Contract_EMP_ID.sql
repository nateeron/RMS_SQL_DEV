...
-- Optional: @DateFrom, @DateTo for invoice date filter (both empty = all).
-- =============================================
DECLARE @Company_ID      INT = 3357,
        @Contract_EMP_ID NVARCHAR(512) = '1786,1787',  -- '' = select all for company
...
WHERE [CON].[Company_ID] = @Company_ID
  AND (
    @Contract_EMP_ID IS NULL
    OR LTRIM(RTRIM(@Contract_EMP_ID)) = ''
    OR [CON].[Contract_EMP_ID] IN (
        SELECT TRY_CAST(LTRIM(RTRIM([value])) AS INT)
        FROM STRING_SPLIT(@Contract_EMP_ID, ',')
        WHERE TRY_CAST(LTRIM(RTRIM([value])) AS INT) IS NOT NULL
    )
  )
)
SELECT cd.Recruiter_ID, cd.Contract_EMP_ID, cd.Candidate_ID, cd.Candidate_Name,
