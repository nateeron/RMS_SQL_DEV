...
SELECT
    ...
    [Payment_Date] = CASE WHEN [PMC].[Payment_Condition_Name] = N'Payment after Guarantee Period'
                          THEN DATEADD(DAY, [MPC].[Payment_After_Guarantee_Period], [CON].[DOJ])
                          ELSE CASE WHEN [PMC].[Payment_Condition_Name] = N'Pay on Candidate Join Date'
                                    THEN [CON].[DOJ]
                                    ELSE [MPC].[Pay_on_Customize_Date] END
                     END,
    [Date_of_Join] = [CON].[DOJ],
    [Date_of_Join_Str] = CASE WHEN [CON].[DOJ] IS NOT NULL THEN FORMAT([CON].[DOJ], N'dd MMM yyyy') ELSE N'-' END,
    ...
    [Salary] = [CON].[Salary],
    [Payment_Condition] = CASE WHEN [PMC].[Payment_Condition_Name] = N'Payment after Guarantee Period'
                               THEN CONCAT(N'Payment after Guarantee ', [MPC].[Payment_After_Guarantee_Period], N' Days')
                               ELSE [PMC].[Payment_Condition_Name] END,
    [Payment_Date_Str] = CASE WHEN CASE WHEN [PMC].[Payment_Condition_Name] = N'Payment after Guarantee Period'
                                       THEN DATEADD(DAY, [MPC].[Payment_After_Guarantee_Period], [CON].[DOJ])
                                       ELSE CASE WHEN [PMC].[Payment_Condition_Name] = N'Pay on Candidate Join Date'
                                                 THEN [CON].[DOJ]
                                                 ELSE [MPC].[Pay_on_Customize_Date] END
                                  END IS NOT NULL
                         THEN FORMAT(CASE WHEN [PMC].[Payment_Condition_Name] = N'Payment after Guarantee Period'
                                         THEN DATEADD(DAY, [MPC].[Payment_After_Guarantee_Period], [CON].[DOJ])
                                         ELSE CASE WHEN [PMC].[Payment_Condition_Name] = N'Pay on Candidate Join Date'
                                                   THEN [CON].[DOJ]
                                                   ELSE [MPC].[Pay_on_Customize_Date] END
...
SELECT cd.Recruiter_ID, cd.Recruiter_Name, cd.Contract_EMP_ID, cd.Candidate_ID, cd.Candidate_Name,
       ...
       cd.Payment_Condition_ID, cd.Payment_Condition_Name, cd.Payment_After_Guarantee_Period, cd.Payment_Date,
       cd.Date_of_Join, cd.Date_of_Join_Str, cd.Commission_Type_ID, cd.Commission_Type_Name, cd.Commission_Value, cd.Currency_ID, cd.Salary,
       cd.Payment_Condition, cd.Payment_Date_Str,
