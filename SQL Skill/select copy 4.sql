USE [Skill]
GO
/****** Object:  StoredProcedure [dbo].[sp_Ins_Import_Skill_byCompany]    Script Date: 1/26/2026 10:35:41 AM ******/
SET ANSI_NULLS ON
GO
...
DECLARE	@Skill_Name NVARCHAR(512) ='F1' ,-- 'Skill_Txxxccc_01',
	@Company_ID INT = 333,
	@User_ID INT = 0,
	-- OUTPUT
	@Map_Skill_ID INT = 0 ,
...
--BEGIN TRY 
	IF (@Skill_Name IS NOT NULL OR @Skill_Name <> '')
		...
		DECLARE @Skill_ID INT = 0,
				...
				-- เช็กว่า ID Company เป็นแม่หรือลูก 
				-- 0 = แม่
		...
		END
		-- ค้นหาว่ามีอยู่หรือยัง Skill_Temp
		...
		IF (@Skill_ID = 0 AND @Skill_Temp_ID = 0)
			BEGIN
			select 'Success ' as stat ;
			select @Skill_Name as Skill_Name;
			select 'INSERT 1' AS INSERT_INTO		
					...
					--SET @Skill_Group_Name = @Skill_Group_ID_INS;
			...
			BEGIN
					...
					ELSE
						BEGIN
								...
								--SET @Skill_By_Comp_ID = SCOPE_IDENTITY();
								SET @Status_Code = '200';
								select 'INSERT 3' AS INSERT_INTO
						END
			END
	END
	ELSE
		...
		select @Map_Skill_ID as Map_Skill_ID
		select @Skill_Group_Name as Skill_Group_Name
		select @Sub_Skill_Group_Name asSub_Skill_Group_Name
		select @Skill_By_Comp_ID as Skill_By_Comp_ID



		select * from [Skill].[dbo].[Skill_By_Company] order by Skill_By_Com_ID desc