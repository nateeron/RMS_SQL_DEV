...
BEGIN TRY   
	...
	--																				AND [COMP].[Is_Active] = 1
	--																				AND [COMP].[Is_Delete] = 0
	--																		) COMPAN ON [COMPAN].[Company_ID] = [MUC].[Company_ID]
	--																WHERE [COMPAN].[Company_ID] IS NOT NULL
	--														   )
	--							AND [MP].[Is_Active] = 1 ) 
	--AND [UL].[Is_Active] = 1;
	...
	--FROM [DBO].[Person] P 
	--LEFT JOIN [Title].[DBO].[Title] T ON [P].[Title_ID] = [T].[Title_ID]
	--LEFT JOIN [dbo].[Map_Person] MPP ON [MPP].[Person_ID] = [P].[Person_ID]
	--LEFT JOIN [User_Login].[DBO].[User_Login] UL ON [UL].[User_Login_ID] = [MPP].[User_Login_ID]
	--WHERE [P].[Person_ID] IN (  SELECT [MP].[Person_ID]
	--							FROM [dbo].[Map_Person] MP
	--							WHERE [MP].[User_Login_ID] IN ( SELECT [MUC].[User_Login_ID]
	--															FROM [User_Login].[DBO].[Map_User_Company] MUC
	--															WHERE [MUC].[Is_Active] = 1
	--															--AND [MUC].[Company_ID] = @Company_ID
	--															)
	--							AND [MP].[Is_Active] = 1 ) 
	--AND [UL].[Is_Active] IS NOT NULL;
