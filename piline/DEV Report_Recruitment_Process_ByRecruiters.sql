...
GO
 ...
 DECLARE @Company_ID INT = 1,
    @Project_Position_ID INT = 7044,
    @DateFromStr VARCHAR(30) =NULL, -- NULL, -- '09 Jan 2026',
    ...
    ;WITH AllData AS (
        ...
        SELECT
            ...
            m.Pipeline_ID,
            m.Company_ID,
            m.Created_Date AS Created_Date1,
            null AS Created_Date2,
        ...
        SELECT
            ...
            h.Pipeline_ID,
            h.Company_ID,
            h.Updated_Date AS Created_Date1,
            h.Created_Date AS Created_Date2,
    ...
    LatestPipelineData AS (
        SELECT
            ...
            SourceType
        FROM Ranked
        WHERE rn = 1 and  (Project_Position_ID = @Project_Position_ID OR @Project_Position_ID = 0)
    ),
	...
	--SELECT * from Ranked
	 ...
	 L.*
	  -- Client and Position info
            ...
            PPI.Position_Name,
            PPI.Job_Req_Date
	 FROM LatestPipelineData L
	 LEFT JOIN ProjectPositionInfo PPI
               ON PPI.Project_Position_ID =  L.Project_Position_ID
	 -- where L.Pipeline_ID = 4
    ...
    ProjectPositionInfo AS (
		...
		SELECT * FROM Pipeline.dbo.Map_Can_Pile_Com m
		 left join ProjectPositionInfo p on p.Project_Position_ID = m.Project_Position_ID
		 where Client_ID = 4121 and m.Project_Position_ID = 7044
	   order by Client_Name,Position_Name;
	   -----------------------------------------------------------------------------
	   SELECT
	...
	ORDER BY [dbo].[Pipeline].[Number_Step] ASC
	   -----------------------------------------------------------------------------
		...
		declare @df varchar(30)= '2026-01-09'
		DECLARE @DateFromT DATETIME = TRY_CONVERT(DATETIME, NULLIF(@df, ''));
		  select @DateFromT
		SELECT * FROM Pipeline.dbo.Map_Can_Pile_Com m 
		where 
		--Project_Position_ID = 7028 and Candidate_ID in (1)--,31
		  Updated_Date >= @DateFromT

        SELECT * FROM Pipeline.dbo.His_Can_Pile_Com h
		where
		--Project_Position_ID = 7028 and Candidate_ID in (1) and
		  Updated_Date >= @DateFromT
		order by Created_Date_His desc