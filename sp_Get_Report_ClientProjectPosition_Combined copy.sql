USE [Pipeline]
GO
/****** Object:  StoredProcedure [dbo].[sp_Get_Report_ClientProjectPosition_Combined]    Script Date: 12/17/2025 10:16:54 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

-- Procedure name: [dbo].[sp_GetClientProjectPosition_Combined]
	-- Function: Combined query from sp_GetList_Com_Client and sp_GetProjectPosition_By_PCID
	-- Create date: 12/11/2025
	-- Description: Get Owner_Name, Position_Name, Project_Position_ID, Client_ID, Client_Name
	-- [dbo].[sp_Get_Report_ClientProjectPosition_Combined] 3357, '3,42,1063','2025-09-28', '2025-10-28'
	-- =============================================
ALTER   PROCEDURE [dbo].[sp_Get_Report_ClientProjectPosition_Combined] 
	@Company_ID INT = 0,
...
BEGIN
	...
	OwnerFilter AS (
		...
		FROM STRING_SPLIT(@Owner_Name, ',')
		WHERE @Owner_Name <> '' AND @Owner_Name IS NOT NULL
	),
	-- CTE: Split Client Name/ID values
	ClientFilter AS (
		SELECT LTRIM(RTRIM([value])) AS [Client_Value]
		FROM STRING_SPLIT(@Client_Name, ',')
		WHERE @Client_Name <> '' AND @Client_Name IS NOT NULL
	),
	-- CTE: Split Position Name values
	PositionFilter AS (
		SELECT LTRIM(RTRIM([value])) AS [Position_Name_Value]
		FROM STRING_SPLIT(@Position_Name, ',')
		WHERE @Position_Name <> '' AND @Position_Name IS NOT NULL
	)
	-- Combined Result
	SELECT 
		CL.Client_ID,
	...
	WHERE 
		...
		(@Owner_Name = '' OR @Owner_Name IS NULL OR 
			EXISTS (
		...
		AND (@Client_Name = '' OR @Client_Name IS NULL OR 
			EXISTS (
		...
		-- Filter by Position Name (if provided) - supports multiple values
		AND (@Position_Name = '' OR @Position_Name IS NULL OR 
			EXISTS (
				SELECT 1 FROM PositionFilter PF
				WHERE [PP].[Position_Name] = PF.[Position_Name_Value]
			)
		)
...
BEGIN CATCH 
	...
	VALUES (
		'1',
		'DB Company - sp_Get_Report_ClientProjectPosition_Combined',
		ERROR_MESSAGE(),
		999,
		GETDATE()
	);
